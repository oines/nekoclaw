...
export function buildDreamPrompt(snapshot: DreamCorpusSnapshot): string {
	return [
		...
		"- If index.md is still only the bootstrap placeholder, replace it with a real global index in this run.",
		...
		"- All route paths written inside index.md must use the worker-facing `.nekoclaw-persona/memory/...` form.",
		"- Do not write maintenance-local relative paths like `memory/...` inside index.md.",
		"- If you see legacy `memory/...` paths in index.md, rewrite them to `.nekoclaw-persona/memory/...` in this run.",
		...
		`- index.md status: ${snapshot.indexHasSubstantiveContent ? "ready" : "bootstrap placeholder"}`,
