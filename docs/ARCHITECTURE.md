...
2. 水化事件（将附件下载到本地工作区）
3. 若已存在配对会话，则先更新会话路由、标题和原始会话日志
4. 检查斜杠命令 → 命令单独处理，不写入人格观察
5. 检查触发规则 → 群组未触发则跳过，不写入人格观察
6. 检查配对状态 → 未配对则进入配对流程，不写入人格观察
7. 只有已配对且进入正常处理路径的消息，才写入人格观察日志
8. 将 `RunJob` 加入队列

**关键设计：** 人格观察不是“看到就记”。只有进入正常对话处理链路的已配对消息才会沉淀为 observation；命令消息、被触发规则忽略的群消息、以及尚未配对的消息都不会进入人格观察。
